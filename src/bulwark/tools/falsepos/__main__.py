"""CLI entry point: `python -m bulwark.tools.falsepos` (ADR-036).

Also reachable as `python -m bulwark_falsepos` via the back-compat shim
in `src/bulwark_falsepos/__init__.py` (ADR-050).
"""
...
from bulwark.tools.falsepos import __version__
from bulwark.tools.falsepos.corpus import categories, load_corpus
from bulwark.tools.falsepos.report import render_json, render_markdown
from bulwark.tools.falsepos.runner import FalseposRunner, stderr_progress


# Repo layout: src/bulwark/tools/falsepos/__main__.py → repo root is parents[4].
_DEFAULT_CORPUS = Path(__file__).resolve().parents[4] / "spec" / "falsepos_corpus.jsonl"
