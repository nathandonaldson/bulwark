...
// ---------------------------------------------------------------------------
// FalsePositive — ADR-036, surfaces bulwark_falsepos in the dashboard.
// ---------------------------------------------------------------------------

function FalsePositive({ store }) {
  const [corpus, setCorpus] = React.useState({size: 0, categories: {}});
  const [reports, setReports] = React.useState([]);
  const [run, setRun] = React.useState({status: 'idle'});
  const [selected, setSelected] = React.useState({
    'deberta-only': true,
    'deberta+promptguard': false,
    'deberta+llm-judge': false,
  });
  const [busy, setBusy] = React.useState(false);

  const presets = [
    {slug: 'deberta-only',         name: 'DeBERTa only'},
    {slug: 'deberta+promptguard',  name: 'DeBERTa + PromptGuard'},
    {slug: 'deberta+llm-judge',    name: 'DeBERTa + LLM Judge', requiresJudge: true},
  ];
  ...
  async function loadCorpus()  { try { setCorpus(await _fetch('/api/falsepos/corpus')); } catch {} }
  async function loadReports() { try { const r = await _fetch('/api/falsepos/reports'); setReports(r.reports || []); } catch {} }
  async function loadStatus()  { try { setRun(await _fetch('/api/falsepos/status')); } catch {} }

  React.useEffect(() => { loadCorpus(); loadReports(); loadStatus(); }, []);
  // Poll while running.
  React.useEffect(() => {
    if (run.status !== 'running') return;
    const id = setInterval(loadStatus, 750);
    return () => clearInterval(id);
  }, [run.status]);
  // Refresh reports when a run completes.
  const wasRunning = React.useRef(false);
  React.useEffect(() => {
    if (wasRunning.current && run.status !== 'running') loadReports();
    wasRunning.current = (run.status === 'running');
  }, [run.status]);

  const judgeReady = !!(store.judge && store.judge.enabled && store.judge.base_url && store.judge.model);

  async function start() {
    setBusy(true);
    try {
      const configs = presets.filter(p => selected[p.slug] && (!p.requiresJudge || judgeReady)).map(p => p.slug);
      if (configs.length === 0) return;
      await _fetch('/api/falsepos/run', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({configs}),
      });
      setRun({status: 'running', completed: 0, total: corpus.size * configs.length});
    } finally { setBusy(false); }
  }

  const running = run.status === 'running';
  const lastResult = (run.status === 'complete') ? run : null;

  return (
    <div>
      <div style={{margin: '24px 0 16px', display: 'flex', alignItems: 'center', gap: 14}}>
        <div style={{flex: 1, height: 1, background: 'var(--border)'}}/>
        <div className="label" style={{margin: 0}}>False-positive sweep</div>
        <div style={{flex: 1, height: 1, background: 'var(--border)'}}/>
      </div>
      ...
      <div className="card" style={{padding: 24}}>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16}}>
          <div>
            <div style={{fontSize: 16, fontWeight: 600}}>
              ...
              <span className="pill" style={{fontSize: 10.5, marginLeft: 8}}>
                {corpus.size || '—'} benign emails
              </span>
            </div>
            <div className="dim" style={{fontSize: 12.5, marginTop: 4, maxWidth: 540}}>
              Sends a curated benign corpus through <span className="mono">/v1/clean</span> for each detector
              configuration and reports the false-positive rate. Companion to the red-team scan above.
              Edit the corpus at <span className="mono">spec/falsepos_corpus.jsonl</span>.
            </div>
          </div>
        </div>

        {Object.keys(corpus.categories || {}).length > 0 && (
          <div style={{display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14}}>
            {Object.entries(corpus.categories).map(([cat, n]) => (
              <span key={cat} className="pill" style={{fontSize: 10.5}}>
                {cat}: {n}
              </span>
            ))}
          </div>
        ...
        <div className="label" style={{marginTop: 4, marginBottom: 8, fontSize: 10}}>Configurations to test</div>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16}}>
          {presets.map(p => {
            const isSelected = !!selected[p.slug];
            const disabled = p.requiresJudge && !judgeReady;
            return (
              <button key={p.slug} onClick={() => !disabled && setSelected(s => ({...s, [p.slug]: !s[p.slug]}))} style={{
                padding: 14, borderRadius: 10, textAlign: 'left',
                ...
                <div style={{display: 'flex', alignItems: 'center', gap: 8}}>
                  ...
                  <span style={{fontSize: 13, fontWeight: 600, color: isSelected ? 'var(--accent)' : 'var(--text)'}}>{p.name}</span>
                </div>
                {disabled && (
                  <div className="dim" style={{fontSize: 11, marginTop: 6}}>Configure the LLM judge first.</div>
                )}
              </button>
            );
          })}
        ...
        {running ? (
          <FalseposProgress run={run} />
        ) : (
          <button className="btn btn-primary" style={{width: '100%', justifyContent: 'center', padding: 10}}
            disabled={busy || corpus.size === 0 || !Object.values(selected).some(Boolean)}
            onClick={start}>
            ▶ Run sweep
          </button>
        )}

        {lastResult && Array.isArray(lastResult.configurations) && (
          <div style={{marginTop: 18}}>
            <div className="label" style={{marginBottom: 8}}>Last sweep — {_fmtDate(lastResult.completed_at)}</div>
            <FalseposResultTable results={lastResult.configurations} corpusSize={corpus.size} />
          </div>
        )}

        <div style={{marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border)'}}>
          <div className="label" style={{marginBottom: 10}}>Past reports</div>
          <FalseposReportsList reports={reports} />
        </div>
      </div>
    </div>
  );
...
function FalseposProgress({ run }) {
  ...
  return (
    <div style={{padding: 14, background: 'var(--bg-2)', borderRadius: 10, border: '1px solid var(--blue-line)'}}>
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8}}>
        <span className="mono tabular" style={{fontSize: 13, color: 'var(--blue)'}}>
          {completed.toLocaleString()} / {(run.total || 0).toLocaleString()} email-runs
        </span>
      ...
      {run.last_id && (
        <div className="dim mono" style={{fontSize: 11, marginTop: 8}}>
          last: <span style={{color: run.last_blocked ? 'var(--red)' : 'var(--text-dim)'}}>{run.last_id}{run.last_blocked ? ' · BLOCKED' : ''}</span>
        </div>
      )}
    </div>
  );
...
function FalseposResultTable({ results, corpusSize }) {
  const ok = results.filter(r => !r.error);
  const ordered = ok.slice().sort((a, b) => (a.false_positive_rate || 0) - (b.false_positive_rate || 0));
  return (
    <div style={{background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 8, padding: 4}}>
      {ordered.map((r, i) => {
        const rate = (r.false_positive_rate || 0) * 100;
        const blocked = r.blocked || 0;
        const total = r.corpus_size || corpusSize || 0;
        const color = rate < 5 ? 'var(--green)' : rate < 15 ? 'var(--amber)' : 'var(--red)';
        return (
          <div key={r.config_slug + i} style={{
            display: 'grid', gridTemplateColumns: 'auto 1fr auto auto', gap: 12, alignItems: 'center',
            padding: '10px 12px',
            borderBottom: i < ordered.length - 1 ? '1px solid var(--hairline)' : 'none',
          }}>
            <span className="dim mono tabular" style={{fontSize: 11}}>#{i + 1}</span>
            <span style={{fontSize: 13, fontWeight: 500}}>{r.config_name || r.config_slug}</span>
            <span className="mono tabular" style={{fontSize: 12, color: 'var(--text-dim)'}}>
              {blocked} / {total}
            </span>
            <span className="mono tabular" style={{fontSize: 13, fontWeight: 600, color}}>
              {rate.toFixed(2)}%
            </span>
          </div>
        );
...
function FalseposReportsList({ reports }) {
  if (!reports || reports.length === 0) {
    return <div className="empty-slate" style={{padding: 20, border: '1px dashed var(--border)', borderRadius: 8, fontSize: 12}}>No false-positive reports yet.</div>;
  }
  return (
    <div className="card" style={{padding: 0, overflow: 'hidden'}}>
      {reports.map((r, i) => (
        <div key={r.filename + i} style={{
          ...
          padding: '12px 14px', borderBottom: i < reports.length - 1 ? '1px solid var(--hairline)' : 'none',
          fontSize: 12,
        }}>
          <span className="dim tabular">{_fmtDate(r.completed_at) || r.filename}</span>
          <span>
            {(r.config_slugs || []).map(slug => (
              <span key={slug} className="pill" style={{fontSize: 10, marginRight: 4}}>{slug}</span>
            ))}
          </span>
          <span className="dim mono" style={{fontSize: 11}}>{r.corpus_size || '?'} emails</span>
          <span className="mono tabular">
            {r.best_fp_rate != null ? `${(r.best_fp_rate * 100).toFixed(2)}%` : '—'}
          </span>
          <a href={`/api/falsepos/reports/${encodeURIComponent(r.filename)}`} className="btn" style={{fontSize: 11}} target="_blank" rel="noopener">JSON</a>
        </div>
      ))}
    </div>
  );
...
Object.assign(window, { PageTest, TraceView, ReportsList, RunProgress, FalsePositive });
